...
SELECT
    ps.speciality,
    COUNT(DISTINCT pt.physician_id) AS physician_count
FROM
    physician_speciality ps
...
WHERE
    ...
    AND pt.physician_id NOT IN (
        SELECT
            DISTINCT pt.physician_id
        FROM
            patient_treatment pt
            JOIN event_category ec ON pt.event_name = ec.event_name
        WHERE
            ec.category = 'Prescription'
    )
GROUP BY
    ps.speciality
ORDER BY
    physician_count DESC;